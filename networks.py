...
class DeepQNetworkDuelingPER:

    def __init__(self, state_size, action_size, learning_rate, name='DeepQNetworkDuelingPER'):
        ...
        with tf.variable_scope(name):
            self.inputs = tf.placeholder(tf.float32, [None, *state_size], name="inputs")
            self.importance_sampling_weights = tf.placeholder(tf.float32, [None, 1], name='importance_sampling_weights')
            self.actions = tf.placeholder(tf.float32, [None, action_size], name="actions")
            self.q_target = tf.placeholder(tf.float32, [None], name="target")
            self.reward = tf.placeholder(tf.float32, (), name="reward")
